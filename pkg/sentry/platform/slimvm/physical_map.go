...
func fillAddressSpace() (excludedRegions []region) {
	...
	//
	// Virtual address range and physical address range should aligned with huge page size.
	requiredAddr, ok := hostarch.Addr(vSize - pSize + hostarch.HugePageSize).RoundUp()
	if !ok {
		panic(fmt.Sprintf(
			"overflow for vSize (%x) - pSize (%x) + HugePageSize(%x)",
			vSize, pSize, hostarch.HugePageSize))
	}
	...
	for filled := uintptr(0); filled < required && current > 0; {
		...
		if errno != 0 {
			...
			// Without this guard, hostarch.Addr(PageSize>>1).RoundUp()
			// rounds back up to PageSize, leaving current stuck and the
			// loop never terminating (mirrors the equivalent guard in
			// the kvm platform's fillAddressSpace).
		...
		// See comment above.
		if filled != required {
			required += hostarch.HugePageSize
		}
	}
	if current == 0 {
...
func computePhysicalRegions(excludedRegions []region) (physicalRegions []physicalRegion) {
	...
	addValidRegion := func(virtual, length uintptr) {
		...
		if offset := virtual &^ hostarch.JumboPageMask; physical&^hostarch.JumboPageMask != offset {
			if newPhysical := (physical & hostarch.JumboPageMask) + offset; newPhysical > physical {
				physical = newPhysical // Round up by only a little bit.
			} else {
				physical = ((physical + hostarch.JumboPageSize) & hostarch.JumboPageMask) + offset
			}
		}
